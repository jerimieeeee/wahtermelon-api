...
class MaternalCareReportService1
{
    ...
    public function get_4anc_give_birth($request)
    {
        return DB::table(function ($query) use ($request) {
            $query->selectRaw("
                ...
                ->havingRaw('trimester1_count >= 1 AND trimester2_count >= 1 AND trimester3_count >= 2');
        })
        ->selectRaw("
                    '4ANC',
                    SUM(
    ...
    public function get_no_of_deliveries_professional($request, $attendant)
    {
        return DB::table('patient_mc_post_registrations')
            ...
            ->when($request->category == 'barangay', function ($q) use ($request) {
                $q->whereIn('facilities.barangay_code', explode(',', $request->code));
            })
    ...
    public function get_no_of_pregnancy_outcome_all($request)
    {
        return DB::table(function ($query) {
            $query->selectRaw("
                    ...
                    facilities.municipality_code AS municipality_code,
                    facilities.barangay_code AS barangay_code
                ")
