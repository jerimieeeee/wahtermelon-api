...
//        }
    ...
    public function _client($methodName = null, $args = null, $jobs = false)
    {
        ...
        try {
            ...
            // Example of making a SOAP call
            if ($methodName && $args) {
                $response = $client->__soapCall($methodName, $args);

                // Clean up the client after the call
                unset($client);
            ...
            return $client; // Return the client if no method is called
    ...
    private function decryptResponse($encryptedOutput)
    {
        ...
        $cipher_key = isset($encryptedOutput->return) ? $credential->cipher_key : PhilhealthCredential::whereProgramCode(request()->program_code)->first()->cipher_key;
    ...
    {
        $credentials = $credential = PhilhealthCredential::query()->whereFacilityCode('DOH000000000048882')->first();
        $credentialsResource = GetTokenResource::make($credentials)->resolve();
        $result = $this->soapMethod('getToken', $credentialsResource);
    ...
    public function encryptData($data, $cipher_key = null, $mimeType = null)
    {
        ...
        //$cipher_key = PhilhealthCredential::select('cipher_key')->whereProgramCode('kp')->pluck('cipher_key')->first();
        $credential = PhilhealthCredential::query()->whereFacilityCode('DOH000000000048882')->first();
        $cipher_key = $cipher_key ?? $credential->cipher_key;
