...
class ParseNHFRFileCommand extends Command
{
    ...
    {
        $data = [
            'facility_code'                 => $properties['Health Facility Code'],
            ...
            'region_id'                     => $properties["Region PSGC"],
            'province_id'                   => $properties["Province PSGC"],
            'municipality_id'               => $properties["City/Municipality PSGC"],
            'barangay_id'                   => $properties["Barangay PSGC"],
        ...
        $data['region_id'] = $this->processPSGC($data, 'region_id', $properties["Region Name"]);
        $data['province_id'] = $this->processPSGC($data, 'province_id', $properties["Province Name"]);
        $data['municipality_id'] = $this->processPSGC($data, 'municipality_id', $properties["City/Municipality Name"]);
        $data['barangay_id'] = $this->processPSGC($data, 'barangay_id', $properties["Barangay Name"]);
    ...
    private function processPSGC($data, $label, $location)
    {
        ...
        if (str_starts_with($data[$label], '18')){
            switch ($label) {
                case 'region_id' && str_starts_with($data['province_id'], '1845'):
                case 'province_id' && str_starts_with($data['province_id'], '1845'):
                case 'municipality_id' && str_starts_with($data['municipality_id'], '1845'):
                case 'barangay_id' && str_starts_with($data['barangay_id'], '1845'):
                    $data[$label] = Str::replaceFirst('18', '06', $data[$label]);
                    break;
                case 'region_id' && str_starts_with($data['province_id'], '1846'):
                case 'province_id' && str_starts_with($data['province_id'], '1846'):
                case 'municipality_id' && str_starts_with($data['municipality_id'], '1846'):
                case 'barangay_id' && str_starts_with($data['barangay_id'], '1846'):
