    ...
    public function toArray($request)
    {
        return [
            '_attributes' => [
                ...
                'pDiagnosticId' => ! empty($this->id) && ! empty($this->laboratory->konsulta_lab_id) ? $this->laboratory->konsulta_lab_id : 0,
                'pOthRemarks' => ! empty($this->id) && empty($this->laboratory->konsulta_lab_id) ? strtoupper($this->laboratory->desc) : '',
                'pIsPhysicianRecommendation' => $this->recommendation_code ?? 'X',
                'pPatientRemarks' => $this->request_status_code ?? 'XX',
                'pReportStatus' => 'U',
                'pDeficiencyRemarks' => '',
            ],
        ];
