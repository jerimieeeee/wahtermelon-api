...
class PatientAbPostExposureResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'booster1_date' => $this->booster1_date,
            'booster1_vaccine_code' => $this->when(! $this->relationLoaded('booster1Vaccine'), $this->animal_type_id),
            'booster1_vaccine' => $this->whenLoaded('booster1Vaccine'),
            'booster1_vaccine_route_code' => $this->when(! $this->relationLoaded('booster1VaccineRoute'), $this->animal_type_id),
            'booster1_vaccine_route' => $this->whenLoaded('booster1VaccineRoute'),
            'booster2_date' => $this->booster2_date,
            'booster2_vaccine_code' => $this->when(! $this->relationLoaded('booster2Vaccine'), $this->animal_type_id),
            'booster2_vaccine' => $this->whenLoaded('booster2Vaccine'),
            'booster2_vaccine_route_code' => $this->when(! $this->relationLoaded('booster2VaccineRoute'), $this->animal_type_id),
            'booster2_vaccine_route' => $this->whenLoaded('booster2VaccineRoute'),
